...
export default function CardInfo() {
  ...
  const getIdDeckInfo = async () => {
    ...
    const championsData = await axios.get(`http://3.36.60.81:8080/champion`);
    const cardData = await axios.get(`http://3.36.60.81:8080/card/${card}`);
    const thumbData = await axios.get(
      `http://3.36.60.81:8080/card/thumb/${thumb}`
    );
    const backColor = await axios.get(
      `http://3.36.60.81:8080/card/trait/${card}`
    );
