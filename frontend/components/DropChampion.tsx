...
import {
  countDownChamp,
  countUpChamp,
} from "../store/actions/numberOfChampAct";
import { http } from "../server";

export default function DropChampion({
  styles,
  indexs,
  setDeckInfo,
  deckInfo,
  ...
  const { image, id } = useSelector(
    (state: RootState) => state.championImgReducer
  );
  ...
  const getchampionTraits = async () => {
    return await axios
      .get(`https://3.34.197.199:8080/champion/traits/${id}`)
      .then(({ data }) => {
  ...
  const removeChampionTraits = async () => {
    const values: number[] = Object.values(deckInfo.final);
    if (
      values.filter((value: number) => value === deckInfo.final[indexs])
        .length < 2
    ) {
      await axios
        .get(`https://3.34.197.199:8080/champion/traits/${deckInfo.final[indexs]}`)
        .then(({ data }) => {
          data.map((trait: string) => {
    ...
    () => ({
      ...
      drop: async () => {
        if (value === "item") {
          ...
          if (
            !keys.filter((index: string) => index === indexs.toString()).length
          ) {
  ...
  return (
    <div
      ...
      style={{
        ...styles,
        backgroundImage: isImage ? `url(${boxImg})` : "none",
        backgroundColor: !isImage ? "gray" : "gray",
      }}
      onDoubleClick={() => {
    ...
    >
      <div ref={drop} className={style.dropChamp}>
        {isOver && <div className={style.overCurser}></div>}
      </div>
      <div className={style.items}>
        {items.map((item, i) => (
          <div key={i}>
            <img className={style.item} src={item} alt="item" />
          </div>
        ))}
      </div>
    </div>
